...
(def player-cards "player/dealer pos assoc with cards in sorted-set (dealer is 0, other decks start at 52) [#{ 8 15 42 46 51} #{1 7 33 43}]"
  (atom []))
(def card-players "card pos assoc with player/dealer num [1 4]"
  (atom []))
...
(def pass-direction "-1=right, 1=left, 2(for now)=across, 0=no passing" (atom -1))
...
(defn pass [] (if (= @pass-direction 0) nil
                (dotimes [x (count @human?)] ()))
  (swap! pass-direction #(condp = % -1 1, 0 -1, 1 2, 2 0)))
